<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SysPartList.aspx.cs" Inherits="Jiazheng.Sys.SysPartList" %>
...
<tr bgcolor="#E7E7E7" class="Title">
	...
	<td height="24" colspan="5">&nbsp;模块列表&nbsp;</td>
...
<tr align='center'>
	...
	<td><%#Eval("Display").ToBoolean().ToChinese()%></td>

	<td>
	    <a href="RoleEdit.aspx?syspartid=<%#Eval("id")%>">权限管理</a> | 
	    <%if (SysPartRole.AllowEdit==true){ %>
	    <a href="SysPartEdit.aspx?id=<%#Eval("id")%>">编辑</a> | 
	    <%} %>
	    <%if (SysPartRole.AllowDelete==true){ %>
	    <a onclick="return confirm('删除后不可恢复，确定要删除吗？')" href="?action=delete&id=<%#Eval("id")%>">删除</a>
	    <%} %>
	</td>
...
<td height="28" colspan="5">
	...
	&nbsp;
	<input id="Button1" type="button" value="新增" onclick="location.href='SysPartEdit.aspx'"  class="coolbg" />
